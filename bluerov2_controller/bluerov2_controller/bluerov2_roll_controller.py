...
class Bluerov2DepthControl(LifecycleNode):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Initializing Bluerov2 roll Control Node...")
        ...
        self.declare_parameter('output_topic', 'controller/roll_force')
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Configuring roll Controller...")
        ...
        self.goal_roll = self.get_parameter('target_roll').value
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Activating roll Controller...")
    ...
    def on_cleanup(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Cleaning up roll Controller...")
        return TransitionCallbackReturn.SUCCESS

    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Shutting down roll Controller...")
        return TransitionCallbackReturn.SUCCESS
    ...
    def _on_set_parameters(self, params):
        for p in params:
            ...
            elif p.name == 'target_roll':
                self.goal_roll = p.value
                self.get_logger().info(f"goal roll changed: {self.goal_roll}")
            elif p.name == 'enable':
                self.enable = p.value
                if self.enable:
                    self.get_logger().info("roll controller enabled")
                else:
                    self.get_logger().info("roll controller disabled")
            elif p.name == 'inverted':
                self.inverted = p.value
                if self.inverted:
                    self.get_logger().info("roll controller inverted")
                else:
                    self.get_logger().info("roll controller not inverted")
        return SetParametersResult(successful=True)


    # Subscriber callback
    def get_target_roll(self,msg):
        self.goal_roll = msg.data
    def _on_roll_measurement(self, msg):
        x = msg.orientation.x
        y = msg.orientation.y
        z = msg.orientation.z
        w = msg.orientation.w
    ...
    def _control_loop(self):
        ...
        if self.hold:
            if self.init_control:
                ...
                self.target = self._current_roll
        else:
            self.target = self.goal_roll

        angle = (self._current_roll + np.pi) % (2 * np.pi) - np.pi
        # self.get_logger().info(f"current roll: {angle}, target roll: {self.target}")
        out,dic = self._pid.update(self.target, self._current_roll) 
        if  self.inverted:
            force = out
        else:
            force = -out
